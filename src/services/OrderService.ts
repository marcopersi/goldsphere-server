...
export class OrderService {
  ...
  async createOrder(request: CreateOrderRequest): Promise<CreateOrderResult> {
    ...
    // Calculate pricing
    const subtotal = this.productService.calculateSubtotal(enrichedItems);
    const calculation = this.calculationService.calculateOrderTotal(subtotal);
    ...
    const order: Order = {
      ...
      fees: {
        processing: calculation.processingFee,
        shipping: 0, // No shipping fees for now
        insurance: 0 // No insurance fees for now
      },
  ...
  private mapDatabaseRowsToOrder(rows: any[]): Order {
    ...
    return {
      ...
      type: OrderType.fromValue(firstRow.type) || OrderType.BUY,
      status: OrderStatus.fromValue(firstRow.status) || OrderStatus.PENDING,
      items,
      ...
      taxes: parseFloat(firstRow.taxes),
      totalAmount: parseFloat(firstRow.totalamount),
      currency: CurrencyEnum.USD,
